...
from bokeh.models import (
    Div, Button, TabPanel, ColumnDataSource,
    DataTable, TableColumn, NumberFormatter, HTMLTemplateFormatter,
    HoverTool
)
...
class RaceTimeTableTab(GT7Tab):
    ...
    def create_components(self):
        ...
        self.export_button = Button(label="Export Times", button_type="success", width=120)
        self.clear_selection_button = Button(label="Clear Selection", button_type="warning", width=120)
        ...
        self.lap_times_source.selected.on_change('indices', self.table_row_selection_callback)
    ...
    def create_lap_times_table(self):
        ...
        time_formatter = HTMLTemplateFormatter(template='<div style="color: <%= value < 0 ? "red" : "green" %>"><%= value %></div>')
        ...
        columns = [
            ...
            TableColumn(field="diff_to_best", title="Δ Best", formatter=time_formatter, width=60),
            TableColumn(field="diff_to_prev", title="Δ Prev", formatter=time_formatter, width=60),
        ]
        ...
        for i, lap in enumerate(laps):
            try:
                ...
                if best_lap_time > 0 and lap_time > 0:
                    ...
                    data["diff_to_best"].append(f"{'+' if diff_to_best > 0 else ''}{diff_to_best/1000:.3f}")
                else:
                ...
                if i > 0 and lap_time > 0:
                    ...
                    if prev_lap_time > 0:
                        ...
                        data["diff_to_prev"].append(f"{'+' if diff_to_prev > 0 else ''}{diff_to_prev/1000:.3f}")
                    else:
    ...
    def export_handler(self, event):
        ...
        with open(filepath, 'w', newline='') as csvfile:
            ...
            # Write header
            writer.writerow(["Lap", "Title", "Time", "Car", "Diff to Best", "Diff to Prev", "S1", "S2", "S3"])
    ...
    def table_row_selection_callback(self, attrname, old, new):
        ...
        if hasattr(self.app, "tab_manager") and hasattr(self.app.tab_manager, "race_tab"):
